...
| Customers | Users, Addresses, Newsletter Subscribers |
...
| System | Roles, Email Settings, Activity Log |
...
view class in `views/__init__.py: DETAIL_VIEW_OVERRIDES`.

## Design notes

* **Money is read-only where the model computes it.** `Order.save()`
  recalculates `discount_amount`, `tax_amount`, `shipping_fee` and
  `final_amount` on every write, and `Product.save()` derives `price` from MRP
  and discount — so those fields are not offered for editing.
...
and a query-count budget that fails if `select_related` regresses.
